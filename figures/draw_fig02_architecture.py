...
def text(x,y,value,size=18,anchor="middle",weight="normal",color=INK):
    s.append(f'<text x="{x}" y="{y}" font-family="Courier New,monospace" font-size="{size}" '
...
def title(x,letter,value):
    text(x,42,f"({letter})",25,"start","bold"); text(x+45,42,value,25,"start","bold")
...
pill(58,88,175,36,"cached prefix Eₖ",BLUE,14); pill(246,88,135,36,"current state Sₜ",GREY,12)
pill(394,88,174,36,"shared flow noise εₖ",GREY,12)
...
rect(48,342,235,70,ORANGE,radius=8); text(165,370,"force-conditioned query",15,weight="bold"); text(165,395,"current prediction",12)
rect(320,342,235,70,BLUE_PALE,radius=8); text(437,370,"missing-force query",15,weight="bold"); text(437,395,"current prediction",12)
...
text(435,532,"missing - aligned reference",10); text(435,547,"+ state rebasing",10)
arrow([(165,415),(165,483)]); arrow([(437,415),(437,483)])
arrow([(437,430),(437,454),(245,454),(245,483)])
pill(316,606,238,36,"reference cached by query k",BLUE,11); text(435,671,"aligned at correction time tⱼ",10)
...
pill(1464,657,254,28,"reference gripper unchanged",BLUE_PALE,11)
