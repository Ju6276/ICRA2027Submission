...
\tikzset{
  ...
  op/.style={font=\sffamily\LARGE\bfseries,text=cTeacher},
  opf/.style={font=\sffamily\large\bfseries,text=cInk!80},
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[stagelab,fill=cInk!7,anchor=west] at (0.20,8.00) {STAGE 1};
  ...
  \node[stagelab,fill=cInk!7,anchor=west] at (5.45,8.00) {STAGE 2};
  ...
  \node[stagelab,fill=cInk!7,anchor=west] at (8.95,8.00) {STAGE 3};
  \node[hero,draw=cTeacher,fill=cTeacher!6,text width=17mm,minimum height=18mm]
    (ctx) at (10.15,6.20) {Shared context $E_k$,\ $\epsilon_k$};
  \node[note] (sched) at (10.15,4.95) {schedule replay $(t,k)$};
  \draw[flow,draw=cInk!55] (sched.north) -- (ctx.south);

  \node[small,draw=cSlow,text width=28mm] (pcond) at (13.05,7.30)
    {force-conditioned\\$A_{T,t|k}^{\mathrm{cond}}$};
  \node[small,draw=cSlow,text width=28mm] (pmiss) at (13.05,6.20)
    {missing-force\\$A_{T,t|k}^{\mathrm{miss}}$};
  \node[small,draw=cSlow,fill=cSlow!5,text width=28mm] (pref) at (13.05,5.10)
    {reference\\$A_k^{\mathrm{ref}}(t_j)$};
  \node[op] at (13.05,6.75) {$-$};
  \node[op] at (13.05,5.65) {$-$};

  \coordinate (fk) at (11.30,6.20);
  \draw[plain,draw=cTeacher] (ctx.east) -- (fk);
  \draw[flow,draw=cTeacher] (fk) |- (pcond.west);
  \draw[flow,draw=cTeacher] (fk) -- (pmiss.west);
  \draw[flow,draw=cTeacher] (fk) |- (pref.west);

  \node[small,draw=cTeacher,fill=cTeacher!8,text width=26mm] (tgtF) at (16.80,6.75)
    {force target\\$\Delta A_{T,t|k}^{\mathrm{force}}$};
  \node[small,draw=cTeacher,fill=cTeacher!8,text width=26mm] (tgtS) at (16.80,5.65)
    {staleness target\\$\Delta A_{T,k\to t}^{\mathrm{stale}}$};
  \draw[flow,draw=cTeacher] (13.35,6.75) -- (tgtF.west);
  \draw[flow,draw=cTeacher] (13.35,5.65) -- (tgtS.west);

  \begin{scope}[on background layer]
    \node[rounded corners=4pt,fill=cInk!4,draw=cInk!18,inner sep=2.5mm,
          fit=(obs)(fh)(enc)(tea)(tag1)] {};
    \node[rounded corners=4pt,fill=cInk!4,draw=cInk!18,inner sep=2.5mm,
          fit=(miss)(tag2)] {};
    \node[rounded corners=4pt,fill=cInk!4,draw=cInk!18,inner sep=2.5mm,
          fit=(ctx)(sched)(pcond)(pref)(tgtF)(tgtS)] {};
  \end{scope}
  ...
  \node[opf] at (12.55,3.00) {$+$};
  \node[opf] at (12.55,2.10) {$+$};
  \node[opf] at (12.55,1.20) {$+$};
