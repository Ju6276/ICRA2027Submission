...
\begin{tikzpicture}[node distance=4mm and 8mm]
  ...
  \node[target,draw=cRes,fill=cRes!9,right=10mm of null] (res)
    {Conditional residual target\\$\Delta A_T^{\mathrm{cond}}=A_T^{\mathrm{full}}\!-\!A_T^{\mathrm{null}}$};
  ...
  \node[bad,below=7mm of fast] (before)
    {prior fast branch: cached prefix,\\action expert per update\\$13$\,Hz};
  \node[good,below=2mm of before] (after)
    {ours: residual only, one pass\\$417$\,Hz \ \ ($31.8\times$)};
