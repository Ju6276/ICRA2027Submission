...
\assumed{Residual-policy training examples are anchored at the $30$~Hz state/action timestamps while retaining every $100$~Hz wrench sample in $\mathcal F_t$. Residual packets may be queried more frequently at deployment; the realized publication rate is measured directly rather than inferred from the action-sample rate within a packet.}

\subsection{Force-Null Mode Optimization}

The learned force-null token and force-null-specific adapter are active only for force-null queries. With the force-conditioned teacher parameters $\theta$ fixed, the force-null parameters $\phi$ are optimized using the teacher's original flow-matching objective,
\begin{equation}
...
\end{equation}
where $\eta$ is flow time and $\epsilon$ is the initial flow noise. Full-force and force-null predictions used to construct a residual target share the same $\epsilon$.
...
\end{equation}
Here $f_A$ defines the spacing between action samples within a packet; residual query times $t_q^F$ are scheduled independently. Step $j$ is held over $[t^F_{q,j},t^F_{q,j}+1/f_A)$, and correction samples are not interpolated. At control time $t$, the executor selects the most recently initiated packet that has completed and remains valid, together with its active step:
\begin{equation}
\begin{aligned}
