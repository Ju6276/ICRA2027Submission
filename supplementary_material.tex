...
\assumed{Residual-policy training examples are anchored at the $30$~Hz state/action timestamps while retaining every $100$~Hz wrench sample in $\mathcal F_t$. Residual queries may be issued more frequently at deployment; the realized correction-update rate is measured directly rather than inferred from the action-sample rate within a chunk.}
...
\section{Asynchronous Reference and Correction Execution}

\subsection{Reference Updates}

A reference query $k$, issued at $t_k$ from state $S_k$, publishes the immutable reference update $(A_{T,k,1:H}^{\mathrm{null}},Z_k^{\mathrm{intent}},S_k,t_k,\bar t_k)$ when its result becomes available at $\bar t_k$. Its samples carry intended execution times
\begin{equation}
...
\end{equation}
An older query that completes later therefore cannot replace a newer active reference update. Samples whose intended times precede $\bar t_{k^*}$ are skipped for direct execution; the cached chunk itself is not truncated. For $t\geq\bar t_{k^*}$, the reference pose $A_{k^*}^{\mathrm{ref}}(t)$ is obtained by timestamp-based interpolation over the original timestamped chunk.

\subsection{Correction Updates}

A residual query $q$, initiated at $t_q^F$, reads $k_q=k^*(t_q^F)$ together with $(\mathcal F_{t_q^F},S_{t_q^F})$. It completes at $\bar t_q^F$ with force-conditioned and staleness correction chunks. Their samples are timestamped as
\begin{equation}
    t^F_{q,j}=t_q^F+(j-1)/f_A,
    \qquad j=1,\ldots,K.
    \label{eq:supp_residual_timestamps}
\end{equation}
Here $f_A$ defines the spacing between correction samples within a chunk; residual query times $t_q^F$ are scheduled independently. Step $j$ is held over $[t^F_{q,j},t^F_{q,j}+1/f_A)$, and correction samples are not interpolated. At control time $t$, the executor selects the most recently initiated residual query whose result has completed and remains valid, together with its active step:
\begin{equation}
\begin{aligned}
