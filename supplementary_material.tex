...
\assumed{The teacher is trained for $50{,}000$ steps with AdamW at a peak learning rate of $2\times10^{-5}$. It predicts $H{=}50$ actions from the latest $100$~ms of the $100$~Hz wrench stream, and flow-matching inference uses $N_{\mathrm{flow}}{=}10$ steps.}
...
As in the main paper, peak force and completion time are reported as mean $\pm$ standard deviation over successful trials pooled across tasks within each platform. Consequently, a task's contribution to the pooled metrics depends on the method's number of successes on that task. For the main comparison, success rate is averaged across the nine tasks, with $20$ trials per method and task. The unseen-object evaluation uses $10$ trials on each of three tasks ($30$ per method), and its success rate is the total successful-trial count divided by $30$. \assumed{The precise per-trial force reduction across arms and completion-time boundaries must be verified against the final evaluation code.}
