...
The correction forward pass takes $2.43$~ms, below the $10$-ms interval between robot command transmissions (Table~\ref{tab:runtime}). The Temporal Teacher and reference-action forward passes both exceed that interval. The shorter computation time allows the correction worker to reuse a reference action instead of waiting for a new generative prediction at each correction query.
...
With the unseen objects, ForceDelta-VLA exceeds the Temporal Teacher by $10$ percentage points in success rate and reduces successful-trial peak force from $21.8$ to $13.6$~N (Table~\ref{tab:physical_generalization}).
